        ...
        >
            <div className="flex flex-col gap-2  overflow-y-auto">
                {items?.map((product, index) => (
                    <Link to={`/product/${product?.product?.slug}?id=${product?.product?.id}`} className="block" key={index}>
                        <div className="flex items-center cursor-pointer gap-2 hover:bg-gray-100 p-1">
                            <div className="flex w-1/6  items-center ">
                                <img src={product?.product.thumbnail} className="w-[70px] h-[70px]  object-contain " alt="" />
                            </div>
                            ...
                            <Price className={'w-1/6 text-red-500 font-semibold'}>{product?.product.price}</Price>
                        </div>
                    </Link>
                ))}
            </div>
